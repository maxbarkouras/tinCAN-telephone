...
Automation and custom scripting is also very easy to develop for this enviornment, as a simple POST request can be used to send a message. Any request sent to ***interface_ip*/set** with the *id* (CAN ID) and *val* (8 byte CAN frame) specified, will be transmitted through the CAN network.
